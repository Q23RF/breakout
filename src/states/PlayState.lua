...
function PlayState:update(dt)
        ...
        for k, brick in pairs(self.bricks) do
            ...
            if brick.inPlay and ball:collides(brick) then
                ...
                if self.score > self.recoverPoints then
                    ...
                    -- the paddle grows wider when we recover
    ...
    for k, powerup in pairs(self.powerups) do
        if powerup:collides(self.paddle) then
            powerup.y = self.paddle.y - 16
            table.remove(self.powerups, k)
            gSounds['paddle-hit']:play()
            if powerup.skin == 10 then
                self.keys = self.keys + 1
            elseif powerup.skin == 3 then
                self.health = math.min(3, self.health + 1)
            else
                for i = 1, 4 do
                    newball = Ball()
                    ...
                    newball.dy = self.balls[1].dy + math.random(-15,15)
                    newball.dx = self.balls[1].dx + math.random(-10,10)
    ...
    -- if all balls are dead, revert to serve state and decrease health
